				...
				uint16_t sw_version, uint8_t cntr_version, uint32_t cntr_flags, char *images_hash)
{
	...
	int ret;
	uint32_t ddr_train_align = sector_size;
	...
	while (img_sp->option != NO_IMG) { /* stop once we reach null terminator */
		...
		case RECOVERY:
			...
			if (img_sp->option == OEI) {
				/**
				 * Align OEI size and the following DDR training
				 * data offset to the required alignment
				 *
				 * file_off     : points to OEI binary offset within OEM container
				 * sbuf.st_size : OEI binary size
				 */
				uint32_t ddr_train_offset = ALIGN(file_off + sbuf.st_size, ddr_train_align);
				size = ddr_train_offset - file_off;
			} else {
				size = ALIGN(sbuf.st_size, sector_size);
			}
		...
		case DUMMY_DDR:
			ddr_train_align = (img_sp->entry ? img_sp->entry : sector_size);
			/* fall through */
		...
		case HOLD:
			...
			file_off += ALIGN(img_sp->entry, ddr_train_align);
			/* Switch back to default value */
			ddr_train_align = sector_size;
			break;
...
img_flags_t parse_image_flags(uint32_t flags, char *flag_list, soc_type_t soc)
{
	...
	case 0xD:
		if (soc == IMX9)
			strcat(flag_list, "DDR QB Dummy image");
		else
	...
	if ((soc == ULP) || (soc == IMX9)) {
		...
		case CORE_ULP_CM33:
			...
			break;
		case CORE_M7_0:
			strcat(flag_list, "CORE_M7_0");
			break;
		case CORE_M7_1:
			strcat(flag_list, "CORE_M7_1");
			break;
		case CORE_ULP_ELE:
		...
		default:
			{
				char temp_str[10];
				sprintf(temp_str, "%d", img_flags.core_id);
				strcat(flag_list, temp_str);
			}
			break;
		}
	} else {
...
void print_image_array_fields(flash_header_v3_t *container_hdrs, soc_type_t soc, bool app_cntr)
{
	...
	for (int i = 0; i < num_images; i++) {
		...
		case 0x3:
			if ((soc == ULP) || (soc == IMX9)) {
				if (img_flags.core_id == CORE_ULP_UPOWER)
					...
					strcpy(img_name, "M33");
				else if ((img_flags.core_id == CORE_M7_0))
					strcpy(img_name, "M7_0");
				else if ((img_flags.core_id == CORE_M7_1))
					strcpy(img_name, "M7_1");
		...
		case 0xD:
			if (soc == IMX9)
				strcpy(img_name, "DDR QB Dummy image");
			else
